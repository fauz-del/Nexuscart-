...
export default function ProductPage() {
  ...
  // FIX: Mandatory Supabase 2026 Public Storage URL structure
  ...
  if (loading) {
    return (
      <div className="h-screen bg-black flex flex-col items-center justify-center text-white font-mono">
        ...
        <p className="tracking-[0.5em] text-xs">ESTABLISHING_LINK...</p>
      </div>
    );
  }
  ...
  if (!product) {
    return (
      <div className="h-screen bg-black flex flex-col items-center justify-center text-white font-mono">
        <p className="tracking-[0.5em] text-xs text-red-500">CRITICAL_ERROR: UNIT_NOT_FOUND</p>
        <button onClick={() => navigate('/shop')} className="mt-8 text-cyan-500 underline text-[10px] cursor-pointer">
          REBOOT_TO_MATRIX
        </button>
      </div>
    );
  ...
  const handleAddToCart = () => {
    addToCart({
      ...
      name: product.name,
      price: product.price,
      // FIX: Passing the full constructed URL to the Cart
      img: `${baseImgUrl}${product.image_url}`, 
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
          <div className="relative aspect-square bg-slate-50 dark:bg-white/5 border border-slate-200 dark:border-white/10 flex items-center justify-center overflow-hidden">
            ...
            <img 
              // FIX: Direct reference to public storage URL
          ...
          <div>
            <div className="flex items-center gap-2 mb-4">
              ...
              <p className="text-[10px] font-mono text-cyan-500 uppercase tracking-[0.3em]">
                {product.category} // UNIT_REF_{product.id.toString().slice(0,8)}
              </p>
            </div>
            ...
            <div className="flex flex-col sm:flex-row items-center gap-4 mb-12">
              <div className="flex items-center border border-slate-200 dark:border-white/10 w-full sm:w-auto h-[60px]">
                <button onClick={() => setQuantity(Math.max(1, quantity - 1))} className="px-6 text-slate-500 hover:text-cyan-500 transition-colors cursor-pointer h-full"><Minus size={14} /></button>
                <span className="w-12 text-center font-mono font-bold dark:text-white text-lg">{quantity}</span>
                <button onClick={() => setQuantity(quantity + 1)} className="px-6 text-slate-500 hover:text-cyan-500 transition-colors cursor-pointer h-full"><Plus size={14} /></button>
              </div>
              ...
              <button 
                ...
                className={`w-full sm:flex-1 h-[60px] font-black uppercase tracking-widest text-xs transition-all flex items-center justify-center gap-3 cursor-pointer ${
                  added ? "bg-green-600 text-white" : "bg-slate-900 dark:bg-white text-white dark:text-black hover:bg-cyan-500 dark:hover:bg-cyan-500 hover:text-white"
                }`}
              >
            ...
            <div className="grid grid-cols-2 gap-8 border-t border-slate-200 dark:border-white/10 pt-8">
               <div className="space-y-2">
                 ...
                 <p className="text-[9px] text-slate-400 font-mono leading-tight">ENCRYPTED_WARRANTY_2YR_ACTIVE</p>
               </div>
               <div className="space-y-2">
                 <div className="flex items-center gap-3 text-[10px] font-black text-slate-900 dark:text-white uppercase tracking-widest">
                   <Truck size={16} className="text-cyan-500" /> Global_Transit
                 </div>
                 <p className="text-[9px] text-slate-400 font-mono leading-tight">INSTANT_DEPLOYMENT_ACTIVE</p>
               </div>
            </div>
          </div>
        </div>
