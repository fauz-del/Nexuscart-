          ...
          {cart.length === 0 ? (
            <div className="text-center py-20 border border-dashed border-slate-300 dark:border-white/10">
              ...
              <button
                ...
                className="bg-slate-900 dark:bg-white text-white dark:text-black px-8 py-3 text-[10px] font-black uppercase tracking-widest hover:bg-cyan-500 transition-all cursor-pointer"
              >
                Return_to_Matrix
              </button>
            </div>
            ...
            <div className="space-y-6">
              {cart.map((item) => (
                <div key={item.id} className="flex flex-col sm:flex-row items-center gap-6 p-6 bg-slate-50 dark:bg-white/5 border border-slate-200 dark:border-white/10">
                  ...
                  <div className="flex items-center border border-slate-200 dark:border-white/10">
                    <button onClick={() => updateQuantity(item.id, -1)} className="p-3 text-slate-500 hover:text-cyan-500 cursor-pointer"><Minus size={14}/></button>
                    <span className="w-8 text-center font-mono font-bold dark:text-white">{item.quantity}</span>
                    <button onClick={() => updateQuantity(item.id, 1)} className="p-3 text-slate-500 hover:text-cyan-500 cursor-pointer"><Plus size={14}/></button>
                  </div>
              ...
              <div className="mt-12 p-8 bg-slate-900 dark:bg-white text-white dark:text-black">
                ...
                <button
                  ...
                  className="w-full py-5 bg-cyan-500 text-white font-black uppercase tracking-widest text-sm hover:bg-cyan-400 transition-all flex items-center justify-center gap-3 cursor-pointer"
                >
