...
export default function Collection() {
  const categories = [
    { 
      name: "VR GEAR", 
      img: vrgoogles1, 
      size: "md:col-span-2 md:row-span-2", // Large Featured Card
      tag: "Immersive" 
    },
    { 
      name: "SMART WATCH", 
    ...
    { 
      name: "GAMING", 
      img: vrgoogles1, // Replace with gaming asset if available
      size: "md:col-span-1 md:row-span-1", 
      tag: "Pro" 
    },
  ];
    ...
    <section className="bg-black py-24 px-6">
      <div className="max-w-7xl mx-auto">
        <div className="flex flex-col md:flex-row md:items-end justify-between mb-12 gap-4">
          ...
          <button className="text-white bg-white/5 border border-white/10 px-6 py-2 rounded-full hover:bg-white/10 transition-colors">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 auto-rows-[300px] gap-4">
          {categories.map((cat, idx) => (
            <div 
              key={idx} 
              className={`relative group overflow-hidden rounded-[2rem] border border-white/5 ${cat.size}`}
            >
